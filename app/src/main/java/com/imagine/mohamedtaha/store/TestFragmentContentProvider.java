...
public class TestFragmentContentProvider extends DialogFragment {
    ...
    private Button BTAddCategory, BTDeleteCategory, bt_galary;
    ...
    private void saveCategory() {
        ...
        if (getArguments() == null && TextUtils.isEmpty(product_name_string)) {
            et_product_name.setError(getString(R.string.error_empty_text));
            et_product_name.requestFocus();
            return;
        }
        if (TextUtils.isEmpty(product_price_string) || Integer.parseInt(product_price_string) <= 0) {
            et_product_price.setError(getString(R.string.error_empty_text_or_zero));
            et_product_price.requestFocus();
            return;
        }
        if (TextUtils.isEmpty(quantity_string) || Integer.parseInt(quantity_string) <= 0) {
            et_product_quantity.setError(getString(R.string.error_empty_text_or_zero));
            et_product_quantity.requestFocus();
            return;
        }
        if (TextUtils.isEmpty(supplier_name_string)) {
            et_supplier_name.setError(getString(R.string.error_empty_text));
            et_supplier_name.requestFocus();
            return;
        }
        if (TextUtils.isEmpty(supplier_phone_string)) {
            et_supplier_phone.setError(getString(R.string.error_empty_text));
            et_supplier_phone.requestFocus();
            return;
        }
